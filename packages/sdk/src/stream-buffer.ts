 ...
 *
 * Stores raw SSE bytes keyed by an arbitrary string (typically a
 * `chatSessionId`) so that a reconnecting client can replay buffered
 * events and continue receiving live events. Every buffered chunk is
 * tagged with a monotonically increasing `seq` so a client can resume
 * from the last frame it has already seen via `?fromSeq=N` style
 * resume.
 *
 * Each server process (agent-runtime pod, API server, custom user
 * service) creates its own `StreamBufferStore` singleton — no shared
 * state between processes. For cross-process durability a higher
 * layer (e.g. a Postgres-backed turn ledger) would wrap this store;
 * this in-memory variant is the hot cache.
 *
...
export class StreamBufferStore {
  ...
  constructor() {
    ...
    // `unref()` exists on Node/Bun's Timeout/Timer but is absent in the
    // browser's number-based setInterval handle. Detect at runtime
    // rather than relying on the type narrowing of `'unref' in ...`,
    // which depends on whether `@types/node` or `@types/bun` is in
    // scope and produces a TS2339 in environments where neither is.
    const handle = this.cleanupTimer as unknown as { unref?: () => void }
