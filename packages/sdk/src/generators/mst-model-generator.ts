...
function mapPrismaToMSTType(field: PrismaField, enums: PrismaEnum[]): string {
  ...
  // `: IAnyModelType` return type on types.late() breaks circular type inference (TS7022/TS7024)
  if (field.kind === 'object') {
    if (field.isList) {
      return `types.array(types.safeReference(types.late((): IAnyModelType => ${field.type}Model)))`
    }
    return `types.safeReference(types.late((): IAnyModelType => ${field.type}Model))`
  }
...
export function generateMSTModel(
  ...
  // Add views — (self: any) breaks circular inference from model chain
  lines.push('  .views((self: any) => ({')
  ...
  // Add actions — Record<string, any> avoids typeof self circular reference
  lines.push('  .actions((self: any) => ({')
  lines.push('    /** Update entity fields */')
  lines.push('    update(changes: Record<string, any>): void {')
