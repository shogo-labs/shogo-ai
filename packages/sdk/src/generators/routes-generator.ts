...
export function generateModelRoutes(
  ...
  const filterFieldNames = getScalarFields(model).map(f => f.name)
  ...
  const lines: string[] = [
    ...
    `const FILTER_FIELDS = ${tsStringArrayLiteral(filterFieldNames)}`,
  ...
  lines.push('      const allowedQueryParams = new Set([...reservedParams, ...FILTER_FIELDS])')
      lines.push('      let where: any = {}')
      lines.push('      ')
      lines.push('      for (const [key, value] of Object.entries(query)) {')
  lines.push('        if (!allowedQueryParams.has(key)) {')
  lines.push('          return sendJson(c, { error: { code: "unknown_query_param", message: `Unknown query parameter "${key}". Supported parameters: ${[...allowedQueryParams].join(", ")}` } }, 400)')
  lines.push('        }')
  ...
  lines.push('      ')
      ...
      lines.push('')
      lines.push('      // Honor an `orderBy` query param of the form "field:direction"')
      lines.push('      // (e.g. "createdAt:desc"). Without this, a `limit` returns an')
      lines.push('      // arbitrary DB-ordered slice rather than the newest/oldest rows.')
      lines.push('      if (typeof query.orderBy === "string" && query.orderBy) {')
      lines.push('        const [field, dir] = query.orderBy.split(":")')
      lines.push('        if (field) orderBy = { [field]: dir === "asc" ? "asc" : "desc" }')
      lines.push('      }')
      lines.push('')
...
/**
 * Generate a concise, agent-readable reference for the generated CRUD API.
 * This is emitted beside the generated route files so agents can inspect the
 * exact pluralization and filter fields instead of guessing endpoint names.
 */
export function generateRoutesDocs(models: PrismaModel[], config: RouteGeneratorConfig = {}): string {
  const basePath = config.basePath ?? '/api'
  const routeModels = models.filter(model => getIdField(model) !== undefined)
  const lines = [
    '# Generated CRUD routes',
    '',
    '> Auto-generated by `shogo generate`; do not edit directly.',
    '',
    'Every model below exposes list/create at the collection path and get/update/delete at `/:id`.',
    'List query parameters are `limit`, `offset`, `userId`, `orderBy`, `include`, plus the model scalar fields.',
    '',
  ]
  for (const model of routeModels) {
    const path = `${basePath.replace(/\/$/, '')}/${toRoutePath(model.name)}`
    const fields = getScalarFields(model).map(field => field.name)
    lines.push(`## ${model.name}`)
    lines.push('')
    lines.push(`- Collection: \`${path}\``)
    lines.push(`- Item: \`${path}/:id\``)
    lines.push('- Methods: `GET`, `POST` on the collection; `GET`, `PATCH`, `DELETE` on an item')
    lines.push(`- List filters: \`${['limit', 'offset', 'userId', 'orderBy', 'include', ...fields].join('`, `')}\``)
    lines.push('')
  }
  return lines.join('\n')
}
