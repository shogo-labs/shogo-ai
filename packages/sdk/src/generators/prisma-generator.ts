 ...
 *
 * `./domain` already re-exports every collection class, model, and type, so a
 * wildcard re-export here keeps `@shogo/domain-stores` barrel exhaustive. Using
 * a narrow named list caused runtime "Cannot read properties of undefined"
 * errors when consumers imported collection classes directly from the barrel.
 */
function generateMSTIndex(_models: PrismaModel[]): string {
  const lines: string[] = [
    GENERATED_FILE_LICENSE_HEADER,
    '/**',
    ' * MST Domain Exports',
