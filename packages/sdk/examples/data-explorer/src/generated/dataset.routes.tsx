...
import type { DatasetHooks } from "./dataset.hooks"

let prisma: PrismaClient | null = null
let hooks: DatasetHooks = {}

export function setPrisma(client: PrismaClient) { prisma = client }
export function setDatasetHooks(h: DatasetHooks) { hooks = h }
...
export function createDatasetRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const items = await p.dataset.findMany({ where, include, take: query.limit ? parseInt(query.limit) : undefined, skip: query.offset ? parseInt(query.offset) : undefined })
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await p.dataset.findUnique({ where: { id } })
      if (!item) return c.json({ error: { code: "not_found", message: "Dataset not found" } }, 404)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await p.dataset.create({ data: body })
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await p.dataset.update({ where: { id }, data: body })
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      await p.dataset.delete({ where: { id } })
