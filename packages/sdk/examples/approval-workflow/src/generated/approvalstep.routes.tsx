...
import type { ApprovalStepHooks } from "./approvalstep.hooks"

let prisma: PrismaClient | null = null
let hooks: ApprovalStepHooks = {}

export function setPrisma(client: PrismaClient) { prisma = client }
export function setApprovalStepHooks(h: ApprovalStepHooks) { hooks = h }
...
export function createApprovalStepRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const items = await p.approvalStep.findMany({ where, include, take: query.limit ? parseInt(query.limit) : undefined, skip: query.offset ? parseInt(query.offset) : undefined })
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await p.approvalStep.findUnique({ where: { id } })
      if (!item) return c.json({ error: { code: "not_found", message: "ApprovalStep not found" } }, 404)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await p.approvalStep.create({ data: body })
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await p.approvalStep.update({ where: { id }, data: body })
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      await p.approvalStep.delete({ where: { id } })
