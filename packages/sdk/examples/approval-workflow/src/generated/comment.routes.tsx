...
export function setCommentHooks(h: CommentHooks) { hooks = h }
...
export function createCommentRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const items = await p.comment.findMany({ where, include, take: query.limit ? parseInt(query.limit) : undefined, skip: query.offset ? parseInt(query.offset) : undefined })
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await p.comment.findUnique({ where: { id } })
      if (!item) return c.json({ error: { code: "not_found", message: "Comment not found" } }, 404)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await p.comment.create({ data: body })
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await p.comment.update({ where: { id }, data: body })
