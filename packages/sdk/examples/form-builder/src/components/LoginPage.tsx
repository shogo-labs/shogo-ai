  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4" style={{ background: 'linear-gradient(135deg, #ec4899 0%, #be185d 100%)' }}>
      <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-md text-center">
        ...
        <p className="text-gray-500 mb-6">Create and share custom forms.</p>
        <form onSubmit={handleSubmit} className="space-y-4 text-left">
          ...
          <button type="submit" disabled={auth.isLoading} className="w-full px-4 py-3 text-white rounded-lg font-medium disabled:opacity-50" style={{ backgroundColor: '#ec4899' }}>
            {auth.isLoading ? 'Please wait...' : mode === 'signin' ? 'Sign In' : 'Create Account'}
          </button>
        </form>
        <p className="mt-6 text-sm text-gray-500">
          {mode === 'signin' ? "Don't have an account? " : 'Already have an account? '}
          <button onClick={() => { setMode(mode === 'signin' ? 'signup' : 'signin'); auth.clearError() }} className="font-medium" style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#ec4899' }}>
