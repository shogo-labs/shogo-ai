...
export class TodoStore {
  ...
  async loadAll(userId?: string, where?: Record<string, unknown>) {
    ...
    try {
      const items = await getTodoList({ data: { userId, where } })
  ...
  async loadById(id: string, userId?: string) {
    try {
      const item = await getTodoById({ data: { id, userId } })
  ...
  /** Create new item with optimistic update */
  async create(input: TodoCreateInput, userId?: string) {
    // Create optimistic item
    const tempId = `temp-${crypto.randomUUID()}`
    ...
    try {
      const item = await createTodo({ data: { input, userId } })
  ...
  async update(id: string, input: TodoUpdateInput, userId?: string) {
    ...
    try {
      const item = await updateTodo({ data: { id, input, userId } })
