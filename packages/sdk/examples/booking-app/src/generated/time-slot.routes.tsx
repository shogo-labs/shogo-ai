...
/**
 * Auto-generated TimeSlot Routes
 *
...
import type { TimeSlotHooks } from "./time-slot.hooks"

// Prisma client instance (injected)
let prisma: PrismaClient | null = null

let hooks: TimeSlotHooks = {}
...
/**
 * Set hooks for TimeSlot routes
 */
export function setTimeSlotHooks(h: TimeSlotHooks) {
  hooks = h
}
...
/**
 * Create TimeSlot CRUD routes
 */
export function createTimeSlotRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[TimeSlot] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.timeSlot.findUnique({
        where: { id },
      })

      if (!item) {
        return c.json({ error: { code: "not_found", message: "TimeSlot not found" } }, 404)
      }
