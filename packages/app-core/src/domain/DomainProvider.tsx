...
/**
 * Hook to optionally access a domain store (doesn't throw if no provider).
 *
 * Use this when you need to check for a domain store but don't want to
 * throw if the DomainProvider is not in the tree (e.g., in useRuntimeStore
 * which falls back to metastore).
 *
 * @param schemaName - The schema name (e.g., "platform-features")
 * @returns The domain store, or undefined if not found or no provider
 */
export function useOptionalDomainStore(schemaName: string): any {
  const ctx = useContext(DomainProviderContext)
