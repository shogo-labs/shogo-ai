 ...
 * 
 * Usage:
 *   bun run src/evals/run-hard-evals.ts [--verbose] [--filter <pattern>]
 * 
...
async function main() {
  ...
  if (filter) {
    ...
    console.log(`\n🔍 Running ${evals.length} evals matching "${filter}"`)
  }
  ...
  console.log(`\n🚀 Running ${evals.length} hard evals...\n`)
  ...
  console.log('\n' + '='.repeat(60))
  console.log('Starting evals... (this may take several minutes)')
  console.log('='.repeat(60) + '\n')
  
  const startTime = Date.now()
  const results = await runEvalSuite('Hard Evals', evals, config)
  ...
  for (const level of [1, 2, 3, 4] as const) {
    ...
    if (levelResults.length > 0) {
      ...
      console.log(`Level ${level}: ${passed}/${levelResults.length} passed (avg: ${avgScore.toFixed(0)}%)`)
    }
  }
