...
export class WebhookAdapter implements ChannelAdapter {
  ...
  async sendMessage(channelId: string, content: string): Promise<void> {
    // Check if this is a reply to a pending sync request
    ...
    // If channelId is a URL, POST the reply there (callback mode)
    ...
    // Otherwise, queue for poll-based retrieval
  ...
  /**
   * Process an inbound webhook message synchronously.
   * Returns the agent's reply text (blocks until agent responds or timeout).
   */
  ...
  /**
   * Retrieve and drain queued outbound messages for a poll-based consumer.
   */
  ...
  /**
   * Register webhook HTTP routes on a Hono app.
   * Called once during server setup (same pattern as WhatsAppAdapter).
   *
   * Routes:
   *   POST   /agent/channels/webhook/incoming   — receive a message
   *   GET    /agent/channels/webhook/outbox/:id  — poll for replies
   *   GET    /agent/channels/webhook/health      — channel health check
   */
  static registerRoutes(app: any, getAdapter: () => WebhookAdapter | null): void {
    ...
    // Outbox polling (for consumers without a callback URL who want to poll)
