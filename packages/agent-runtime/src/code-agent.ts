...
export async function executeCodeAgentTask(
  ...
  writeFileSync(claudeMdPath, [
    ...
    'You build custom agent interfaces — web UIs that connect back to the parent Shogo agent.',
    '',
    ...
    '',
    '// Subscribe to canvas surface updates (SSE)',
    'const { surfaces, dispatchAction } = useCanvasStream()',
    '```',
    '',
    'Or use the client directly: `new AgentClient()` with relative URLs (`/agent/status`, `/agent/chat`, etc.).',
    '',
    '## MANDATORY: Use Templates for Scaffolding',
    '',
    'You have MCP tools: `mcp__shogo__template_list` and `mcp__shogo__template_copy`.',
    '',
    '**Required workflow for any new project:**',
    ...
    '',
    'Agentic templates (preferred): agent-dashboard, approval-workflow, data-explorer.',
    'Other templates: todo-app, expense-tracker, crm, inventory, kanban, ai-chat, form-builder, feedback-form, booking-app.',
    '',
    '## Environment',
    '',
...
export function createCodeAgentTool(config: CodeAgentConfig): AgentTool {
  return {
    name: 'code_agent',
    description:
      'Delegate a coding task to the Claude Code sub-agent. Use this in "app" mode for ALL code creation, modification, and debugging. The sub-agent builds custom agent interfaces — web UIs that connect back to your agent runtime via @shogo-ai/sdk/agent (useAgentStatus, useAgentChat, useCanvasStream). It has full filesystem access to the project/ directory and can use templates to scaffold. NEVER write application code yourself — always delegate to this tool.',
    label: 'Code Agent',
