...
function collectFiles(dir: string, base = dir): Record<string, string> {
  ...
  // which previously made the generated YAML's `files` key order — and so
  // its byte-for-byte content — differ between a local macOS run and Linux
  // CI, tripping the "no drift" test in issue-pipeline-template.test.ts on
  // one platform but not the other. Sort so output is identical everywhere.
