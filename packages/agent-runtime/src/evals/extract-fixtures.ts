...
function extractSkillFixtures() {
  return {
    ...
    examples: [
      ...
      { id: 'create-github-pr-check', type: 'create', user_description: 'Create a skill that checks my GitHub PRs and summarizes review status', expected_skill_name: 'github-pr-check', expected_trigger_phrases_min: 3, expected_tools: ['web_fetch', 'memory_write'], optimal_tool_calls: 1, optimal_iterations: 1 },
...
function extractMemoryFixtures() {
  return {
    ...
    examples: [
      ...
      { id: 'ephemeral-currency-conversion', source_test: null, user_message: 'Convert 1,500 USD to EUR at today\'s exchange rate.', conversation_summary: 'User asked for a one-off currency conversion — should use web_fetch but not persist', tools_used: ['web_fetch'], current_memory: '# Memory\n', has_session_context: false, expected_write: false, expected_target_file: '', expected_content: '', expected_retrieval_tool: 'none', expected_retrieval_query: '', optimal_tool_calls: 1, optimal_iterations: 1 },
