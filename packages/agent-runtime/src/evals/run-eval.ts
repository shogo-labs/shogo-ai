...
import { UNIFIED_CONNECT_EVALS } from './test-cases-unified-connect'
...
import { TOOL_DISCIPLINE_EVALS } from './test-cases-tool-discipline'
import { TYPED_BUILD_EVALS } from './test-cases-typed-build'
...
import { ORCHESTRATION_RESILIENCE_EVALS } from './test-cases-orchestration-resilience'
...
function getEvals(track: string): AgentEval[] {
  switch (track) {
    ...
    case 'unified-connect': return UNIFIED_CONNECT_EVALS
    ...
    case 'tool-discipline': return TOOL_DISCIPLINE_EVALS
    case 'typed-build': return TYPED_BUILD_EVALS
    ...
    case 'orchestration-resilience': return ORCHESTRATION_RESILIENCE_EVALS
    ...
    case 'agentic': return [...BUSINESS_USER_EVALS, ...STARTUP_CTO_EVALS, ...FREELANCER_EVALS, ...CONTENT_CREATOR_EVALS, ...NONPROFIT_EVALS, ...EVENT_PLANNER_EVALS, ...ADVERSARIAL_EVALS, ...CROSS_CUTTING_EVALS, ...SUBAGENT_COORDINATION_EVALS, ...TEAMMATE_COORDINATION_EVALS, ...ORCHESTRATION_RESILIENCE_EVALS]
...
async function runEvalOnWorker(
  ...
  try {
    const result = await runEval(ev, {
      ...
      timeoutMs: Number.isFinite(timeoutMsArg) && timeoutMsArg > 0 ? timeoutMsArg : 1_200_000,
      verbose: verboseFlag,
