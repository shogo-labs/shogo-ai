...
export class PreviewManager {
   ...
   * (the extra entry is harmless).
   */
  private ensureViteWatchConfig(cwd: string): string {
    const shogoDir = join(cwd, '.shogo')
    try { mkdirSync(shogoDir, { recursive: true }) } catch { /* exists */ }
    const configPath = join(shogoDir, 'vite.watch.config.ts')
    const wrapper = [
    ...
    writeFileSync(configPath, wrapper, 'utf-8')
    return configPath
  }
  ...
  private async startBuildWatch(): Promise<void> {
    ...
    // Generate a wrapper config that adds .shogo/ to Vite's watch
    // ignore list — prevents the build-log infinite loop where
    // emitBuildLine → scheduleLogWrite → appendFile triggers
    // chokidar → rebuild → more stdout → more writes.
    const watchConfigPath = this.ensureViteWatchConfig(cwd)
    ...
    try {
      viteProcess = spawn(
        isWindows ? `"${invocation.cmd}"` : invocation.cmd,
        [...invocation.argsPrefix, 'build', '--watch', '--emptyOutDir', 'false', '--config', watchConfigPath, ...this.viteBaseArgs()],
        {
          cwd,
          stdio: ['ignore', 'pipe', 'pipe'],
