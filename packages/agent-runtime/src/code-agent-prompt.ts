...
1. **Explore first** — Use \`ls\`, \`glob\`, \`grep\`, and \`search\` to understand the project structure and existing patterns before touching anything.
...
Before your first edit, you MUST run at least one exploratory command (\`ls\`, \`grep\`, \`glob\`, or \`search\`) to understand the project structure. Never edit code you have not read.
...
**When NOT to use search (use grep instead):**
...
1. **Explore first** — Use \`ls\`, \`glob\`, \`grep\`, and \`search\` to understand the project structure before touching anything.
