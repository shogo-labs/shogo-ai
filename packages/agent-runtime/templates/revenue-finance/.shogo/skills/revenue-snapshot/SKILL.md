...
tools: [search_integrations, connect, memory_write]
---
...
1. **Connect** — Check if Stripe is installed via `search_integrations`. If not:
   - `connect({ name: "stripe" })` to connect via Composio OAuth
...
If Stripe isn't available, ask the user to provide revenue data manually or suggest connecting another payment tool via `search_integrations("payments")`.
