...
export const COMPONENT_SCHEMA: ComponentSchema[] = [
  ...
  { type: 'DataList', category: 'data', description: 'Repeating template container with optional client-side search filtering', hasChildren: true, props: { emptyText: str('Empty state text'), filterPath: str('JSON Pointer to search term in data model for client-side filtering'), filterFields: arr('Item fields to match against the search term'), className: CLASS_PROP } },
  ...
  { type: 'TextField', category: 'interactive', description: 'Text input', hasChildren: false, props: { label: str('Field label'), placeholder: str('Placeholder'), value: str('Initial value'), type: str('Input type', { enum: ['text', 'email', 'password', 'number', 'url', 'tel'], default: 'text' }), disabled: bool('Disable field'), action: ACTION_PROP, dataPath: str('JSON Pointer for data binding'), debounceMs: num('Debounce delay in ms before writing to data model'), className: CLASS_PROP } },
