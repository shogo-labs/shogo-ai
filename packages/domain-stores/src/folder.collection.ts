...
/**
 * Auto-generated Folder Collection
 *
...
import { FolderModel, type IFolder, type IFolderSnapshotIn } from "./folder.model"
 ...
 */
export const FolderCollection = types
  ...
  .views(self => ({
    ...
    /** Get item by ID */
    get(id: string): IFolder | undefined {
    ...
    filter(predicate: (item: IFolder) => boolean): IFolder[] {
      return this.all.filter(predicate)
    },

    /** Find first item matching predicate */
    find(predicate: (item: IFolder) => boolean): IFolder | undefined {
  ...
  .actions(self => {
    ...
    return {
      ...
      addItem(item: IFolderSnapshotIn) {
        self.items.put(item)
      },
       ...
       */
      create: flow(function* (input: Partial<IFolderSnapshotIn>) {
       ...
       */
      update: flow(function* (id: string, changes: Partial<IFolderSnapshotIn>) {
        const existing = self.items.get(id)
        if (!existing) {
...
const relationFields = ["workspace","parent","children","projects"]
...
// Type exports
export type IFolderCollection = Instance<typeof FolderCollection>
