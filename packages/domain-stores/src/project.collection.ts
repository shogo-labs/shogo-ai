...
/**
 * Auto-generated Project Collection
 *
...
import { ProjectModel, type IProject, type IProjectSnapshotIn } from "./project.model"
 ...
 */
export const ProjectCollection = types
  .model("ProjectCollection", {
    items: types.map(ProjectModel),
  ...
  .views(self => ({
    ...
    filter(predicate: (item: IProject) => boolean): IProject[] {
      return this.all.filter(predicate)
    },

    /** Find first item matching predicate */
    find(predicate: (item: IProject) => boolean): IProject | undefined {
  ...
  .actions(self => {
    ...
    return {
      ...
      addItem(item: IProjectSnapshotIn) {
        self.items.put(item)
      },
       ...
       */
      create: flow(function* (input: Partial<IProjectSnapshotIn>) {
       ...
       */
      update: flow(function* (id: string, changes: Partial<IProjectSnapshotIn>) {
        const existing = self.items.get(id)
        if (!existing) {
...
function transformForMST(obj: any): any {
  ...
  const dateFields = ["createdAt","updatedAt","publishedAt","publishStatusAt","lastMessageAt"]
...
// Type exports
export type IProjectCollection = Instance<typeof ProjectCollection>
