...
/**
 * Auto-generated Member Collection
 *
...
import { MemberModel, type IMember, type IMemberSnapshotIn } from "./member.model"
 ...
 */
export const MemberCollection = types
  .model("MemberCollection", {
    items: types.map(MemberModel),
  ...
  .views(self => ({
    ...
    /** Get item by ID */
    get(id: string): IMember | undefined {
    ...
    filter(predicate: (item: IMember) => boolean): IMember[] {
      return this.all.filter(predicate)
    },

    /** Find first item matching predicate */
    find(predicate: (item: IMember) => boolean): IMember | undefined {
  ...
  .actions(self => {
    ...
    return {
      ...
      addItem(item: IMemberSnapshotIn) {
        self.items.put(item)
      },
       ...
       */
      create: flow(function* (input: Partial<IMemberSnapshotIn>) {
       ...
       */
      update: flow(function* (id: string, changes: Partial<IMemberSnapshotIn>) {
        const existing = self.items.get(id)
        if (!existing) {
...
// Type exports
export type IMemberCollection = Instance<typeof MemberCollection>
