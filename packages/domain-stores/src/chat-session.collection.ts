...
/**
 * Auto-generated ChatSession Collection
 *
...
import { ChatSessionModel, type IChatSession, type IChatSessionSnapshotIn } from "./chat-session.model"
 ...
 */
export const ChatSessionCollection = types
  ...
  .views(self => ({
    ...
    get all(): IChatSession[] {
      return Array.from(self.items.values())
    },

    /** Get item by ID */
    get(id: string): IChatSession | undefined {
    ...
    filter(predicate: (item: IChatSession) => boolean): IChatSession[] {
      return this.all.filter(predicate)
    },

    /** Find first item matching predicate */
    find(predicate: (item: IChatSession) => boolean): IChatSession | undefined {
  ...
  .actions(self => {
    ...
    return {
      ...
      addItem(item: IChatSessionSnapshotIn) {
        self.items.put(item)
      },
       ...
       */
      create: flow(function* (input: Partial<IChatSessionSnapshotIn>) {
       ...
       */
      update: flow(function* (id: string, changes: Partial<IChatSessionSnapshotIn>) {
        const existing = self.items.get(id)
        if (!existing) {
...
const relationFields = ["project","workspace","attachedProjects","messages","toolCallLogs","agentTasks","plans"]
...
function transformForMST(obj: any): any {
  ...
  const dateFields = ["createdAt","updatedAt","lastActiveAt","activeTurnStartedAt","activeTurnHeartbeatAt"]
...
// Type exports
export type IChatSessionCollection = Instance<typeof ChatSessionCollection>
