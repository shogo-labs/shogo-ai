...
/**
 * Auto-generated ToolCallLog Collection
 *
...
import { ToolCallLogModel, type IToolCallLog, type IToolCallLogSnapshotIn } from "./tool-call-log.model"
 ...
 */
export const ToolCallLogCollection = types
  .model("ToolCallLogCollection", {
    items: types.map(ToolCallLogModel),
  ...
  .views(self => ({
    ...
    get all(): IToolCallLog[] {
      return Array.from(self.items.values())
    },

    /** Get item by ID */
    get(id: string): IToolCallLog | undefined {
    ...
    filter(predicate: (item: IToolCallLog) => boolean): IToolCallLog[] {
      return this.all.filter(predicate)
    },

    /** Find first item matching predicate */
    find(predicate: (item: IToolCallLog) => boolean): IToolCallLog | undefined {
  ...
  .actions(self => {
    ...
    return {
      ...
      addItem(item: IToolCallLogSnapshotIn) {
        self.items.put(item)
      },
       ...
       */
      create: flow(function* (input: Partial<IToolCallLogSnapshotIn>) {
       ...
       */
      update: flow(function* (id: string, changes: Partial<IToolCallLogSnapshotIn>) {
        const existing = self.items.get(id)
        if (!existing) {
...
// Type exports
export type IToolCallLogCollection = Instance<typeof ToolCallLogCollection>
