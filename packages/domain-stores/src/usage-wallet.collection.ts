...
/**
 * Auto-generated UsageWallet Collection
 *
...
import { UsageWalletModel, type IUsageWallet, type IUsageWalletSnapshotIn } from "./usage-wallet.model"
import type { ISDKEnvironment } from "@shogo-ai/sdk"

// ============================================================================
// UsageWallet Collection
// ============================================================================

const ENDPOINT = "/api/usage-wallets"

/**
 * UsageWallet Collection Model
 *
 * Features:
 ...
 */
export const UsageWalletCollection = types
  .model("UsageWalletCollection", {
    items: types.map(UsageWalletModel),
  ...
  .views(self => ({
    ...
    get all(): IUsageWallet[] {
      return Array.from(self.items.values())
    },

    /** Get item by ID */
    get(id: string): IUsageWallet | undefined {
    ...
    filter(predicate: (item: IUsageWallet) => boolean): IUsageWallet[] {
      return this.all.filter(predicate)
    },

    /** Find first item matching predicate */
    find(predicate: (item: IUsageWallet) => boolean): IUsageWallet | undefined {
  ...
  .actions(self => {
    ...
    return {
      ...
      addItem(item: IUsageWalletSnapshotIn) {
        self.items.put(item)
      },
       ...
       */
      create: flow(function* (input: Partial<IUsageWalletSnapshotIn>) {
       ...
       */
      update: flow(function* (id: string, changes: Partial<IUsageWalletSnapshotIn>) {
        const existing = self.items.get(id)
        if (!existing) {
...
function transformForMST(obj: any): any {
  ...
  const dateFields = ["lastDailyReset","lastMonthlyReset","fiveHourWindowStart","weeklyWindowStart","createdAt","updatedAt"]
...
// Type exports
export type IUsageWalletCollection = Instance<typeof UsageWalletCollection>
