...
export const TEST_MAINTENANCE = `## Test Maintenance

When you modify the application (add features, change UI, add models):

1. **Generated tests** (\`tests/e2e.generated.test.ts\`) are auto-generated by \`bunx shogo generate\`. NEVER edit this file manually. It contains baseline CRUD tests derived from the Prisma schema.

2. **User tests** (\`tests/e2e.test.ts\`) are owned by the user. When you change the app:
   - If you add a new UI feature, add a test for it in \`tests/e2e.test.ts\`
   - If you change form fields or button text, update existing tests to match
   - If you add a new page/route, add a navigation test

3. **After schema changes** that trigger \`bun run generate\`, the generated tests will auto-update to match the new models and fields. But update \`tests/e2e.test.ts\` if custom features changed.

4. **Test patterns to follow** (use Playwright best practices):
   - \`page.getByRole('button', { name: /text/i })\` for buttons
   - \`page.getByPlaceholder('...')\` for inputs
   - \`expect(...).toBeVisible({ timeout: 5000 })\` for assertions
   - Use the \`ensureLoggedIn\` helper from the generated tests for auth setup`
