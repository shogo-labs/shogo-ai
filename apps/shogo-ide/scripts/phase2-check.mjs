...
assert(!ideViews.includes("'--disable-extensions'"), 'managed workbench must not blanket-disable extensions because Shogo Agent Chat is bundled as an extension')
assert(ideViews.includes('--extensionDevelopmentPath'), 'managed workbench must load bundled Shogo extensions')
assert(ideViews.includes('shogo-agent-chat'), 'managed workbench must load the Shogo Agent Chat extension')
