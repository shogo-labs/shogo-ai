...
function buildUserData(config: CloudInitConfig): string {
  ...
  if (config.useBundleMount) {
    ...
    lines.push('  - |')
    lines.push('    echo "=== VirtioFS mount diagnostics ==="')
    lines.push('    mount -t virtiofs workspace /workspace 2>&1 || echo "WARN: workspace virtiofs mount failed"')
    lines.push('    mount -t virtiofs bundle /mnt/bundle 2>&1 || echo "WARN: bundle virtiofs mount failed"')
    lines.push('    echo "workspace mount:"; ls /workspace/ 2>&1 | head -5')
    lines.push('    echo "bundle mount:"; ls /mnt/bundle/ 2>&1 | head -10')
    lines.push('    echo "all mounts:"; mount | grep -E "virtiofs|workspace|bundle" 2>&1')
    lines.push('    echo "=== end diagnostics ==="')
