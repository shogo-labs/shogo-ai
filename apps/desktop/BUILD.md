...
`publish-agent-runtime.yml`, `publish-shogo-worker.yml`) — no CLI or worker
runtime assets are published for a desktop beta build.

### Beta-only dry run (no stable users touched)

Use this to validate the full update path end to end after changing anything
...
3. Manually run both workflows again for a newer `main` commit, with
   ...
   Squirrel.Mac / Squirrel.Windows download + signature-verification path
   against the `releases.shogo.ai` Worker feed, not a mock.
4. Switch back to **Stable** in Settings: confirm no downgrade is offered and
   the app stays on the installed beta build until a stable tag with a higher
   version is published — then confirm that stable tag *is* offered.
5. Rollback check: delete the newest beta prerelease on GitHub. The Worker
   falls back to the previous one; the next probe on a client still offering
   the deleted version clears the banner (existing `runProbe()` behavior —
   see `apps/desktop/src/updater.ts`).

Before step 3, `./scripts/check-desktop-feed.sh` is a faster (read-only) way
to confirm the Worker itself is resolving the new prerelease correctly,
without needing an installed beta build.
