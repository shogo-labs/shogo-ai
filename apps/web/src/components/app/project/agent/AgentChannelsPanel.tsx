...
function WebhookDetails({
  ...
  const sendTest = async () => {
    ...
    try {
      ...
      loadActivity() // Refresh activity after test
    } catch (err: any) {
...
export function AgentChannelsPanel({ projectId, visible, localAgentUrl }: AgentChannelsPanelProps) {
  ...
  const loadChannels = useCallback(async () => {
    ...
    try {
      ...
      // Auto-expand webhook on first load only
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {ch.type === 'webhook' && ch.connected && !isExpanded && (
                            <div className="text-[11px] text-muted-foreground mt-0.5">
                              {(ch.metadata as any)?.messageCount || 0} messages • Click to expand
                            </div>
                          )}
                        </div>
                        <div className="flex items-center gap-1.5">
