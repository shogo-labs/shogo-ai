/**
 * HostMSTDemo - Isomorphic Meta-Store Demo with Full CRUD
 *
...
const DynamicSchemaDemo = observer(function DynamicSchemaDemo() {
  ...
  async function loadSchema() {
    try {
      ...
      console.log('[HostMSTDemo] Schema loaded:', loadedSchema.name)
      ...
      console.log('[HostMSTDemo] Loading existing data from server...')
      await runtimeStore.pageCollection.loadAll()
      console.log('[HostMSTDemo] Data loaded:', runtimeStore.pageCollection.all().length, 'pages')

    } catch (err: any) {
      console.error('[HostMSTDemo] Error:', err)
...
const DemoContent = observer(function DemoContent({
  ...
  const handleAddPage = async () => {
    ...
    console.log('[HostMSTDemo] Added and saved page:', title)
  }

  const handleRemovePage = (id: string) => {
    store.pageCollection.remove(id)
    setSaveStatus('Unsaved changes')
    console.log('[HostMSTDemo] Removed page:', id)
  }
  ...
  const handleToggleStatus = async (page: any) => {
    ...
    console.log('[HostMSTDemo] Updated and saved status:', page.title, '->', newStatus)
  }
  ...
  const handleLoad = async () => {
    try {
      ...
      console.log('[HostMSTDemo] Loading from MCP server...')

      await store.pageCollection.loadAll()

      setSaveStatus('Loaded from server!')
      console.log('[HostMSTDemo] Loaded from MCP server!')

      // Clear success message after 2 seconds
      setTimeout(() => setSaveStatus(null), 2000)
    } catch (err: any) {
      console.error('[HostMSTDemo] Load error:', err)
