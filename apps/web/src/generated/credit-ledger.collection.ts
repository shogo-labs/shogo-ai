/**
 * Auto-generated CreditLedger Collection
 *
...
import { CreditLedgerModel, type ICreditLedger, type ICreditLedgerSnapshotIn } from "./credit-ledger.model"
 ...
 */
export const CreditLedgerCollection = types
  .model("CreditLedgerCollection", {
    items: types.map(CreditLedgerModel),
  ...
  .views(self => ({
    ...
    get all(): ICreditLedger[] {
      return Array.from(self.items.values())
    },

    /** Get item by ID */
    get(id: string): ICreditLedger | undefined {
    ...
    filter(predicate: (item: ICreditLedger) => boolean): ICreditLedger[] {
      return this.all.filter(predicate)
    },

    /** Find first item matching predicate */
    find(predicate: (item: ICreditLedger) => boolean): ICreditLedger | undefined {
  ...
  .actions(self => {
    ...
    return {
      ...
      addItem(item: ICreditLedgerSnapshotIn) {
        self.items.put(item)
      },
       ...
       */
      create: flow(function* (input: Partial<ICreditLedgerSnapshotIn>) {
       ...
       */
      update: flow(function* (id: string, changes: Partial<ICreditLedgerSnapshotIn>) {
        const existing = self.items.get(id)
        if (!existing) {
...
function transformForMST(obj: any): any {
  ...
  const dateFields = ["createdAt", "updatedAt", "expiresAt", "publishedAt", "readAt", "emailSentAt", "lastActiveAt", "lastDailyReset", "lastMonthlyReset"]
...
// Type exports
export type ICreditLedgerCollection = Instance<typeof CreditLedgerCollection>
