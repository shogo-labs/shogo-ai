...
export interface PlanStreamContextValue {
  ...
  /** Lifecycle of the stakeholder summary for the current streaming plan. */
  summaryStatus: PlanSummaryStatus
  /** Final summary markdown once generation completes. */
  ...
  setSummaryStatus: (s: PlanSummaryStatus) => void
  setStreamingSummary: (v: string | null) => void
...
export function PlanStreamProvider({ children }: PlanStreamProviderProps) {
  ...
  const [summaryStatus, setSummaryStatus] = useState<PlanSummaryStatus>("idle")
