...
import { AtSign, X, FileText, File, Folder, Image as ImageIcon } from "lucide-react-native"
...
function iconFor(ext: string | undefined) {
  if (!ext) return <Folder size={12} className="text-primary" />
...
export function FileMentionChip({
  ...
  return (
    <View className={cn("relative", className)}>
      <View
        className={cn(
          "flex-row items-center gap-1.5 rounded-xl border px-2 py-1.5 max-w-[260px]",
          status === "warning"
      ...
      >
        ...
        <View className="h-5 w-5 items-center justify-center rounded-md bg-primary/15">
