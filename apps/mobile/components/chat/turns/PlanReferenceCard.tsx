 ...
 *
 * Native phone: compact summary card. The composer variant sits above the
 * chat input. Tapping any variant opens the Plans panel; Build remains a
 * separate action for the pending composer plan.
...
export interface PlanReferenceCardProps {
  ...
  isUpdate: boolean
  /** Shows a loading affordance while the tool call is still streaming. */
  isStreaming?: boolean
  /** Opens the Plans tab. */
  onViewPlan?: () => void
...
export function PlanReferenceCard({
  ...
  const [buildModelId] = usePlanBuildModel(selectedModel, planKey)
  const canBuild = !isConfirmed && !!onBuild
  const label = isUpdate ? "Updated plan" : "Created plan"
    ...
    return (
      ...
      >
        ...
        {isConfirmed ? (
          ...
          <ChevronRight className="text-muted-foreground" size={PHONE_DENSITY.icon.md} />
        ) : null}
      </View>
    )
  }
