...
describe("groupWorkParts", () => {
  ...
  test("groups consecutive image generations into a gallery", () => {
    const parts = [
      tool("generate_image", { prompt: "portrait", size: "1024x1792" }),
      reasoning("keep the candidates together"),
      tool("generate_image", { prompt: "landscape", size: "1792x1024" }),
    ];
    const grouped = groupWorkParts(parts);
    expect(grouped).toHaveLength(1);
    expect(grouped[0].type).toBe("image-gallery");
    if (grouped[0].type === "image-gallery") {
      expect(grouped[0].tools).toHaveLength(2);
    }
  });

  test("keeps a lone image generation as a dedicated tool", () => {
    const grouped = groupWorkParts([tool("generate_image", { prompt: "one" })]);
    expect(grouped).toHaveLength(1);
    expect(grouped[0].type).toBe("tool");
  });
...
describe("partitionTurn", () => {
  test("everything before the LAST text part is the work log; the rest is the final segment", () => {
    ...
    expect(workLog.map((p) => p.type)).toEqual([
      "work-group",
      "text",
      "work-group",
    ]);
  ...
  test("no text part at all: everything is the final segment, work log is empty", () => {
    const parts = [
      tool("Read", { path: "/a.ts" }),
      tool("exec", { command: "ls" }, "streaming"),
    ];
    const grouped = groupWorkParts(parts);
    const { workLog, finalSegment } = partitionTurn(grouped);
    ...
    const message = msg([
      {
        ...
        data: { startedAt: 2000, completedAt: 8000 },
      },
    ]);
    const timing = extractTurnTiming(message);
