 ...
 * "Running..." line.
 */
function deriveLatestActivityLabel(
...
function extractGeneratedImageTools(
  parts: readonly any[] | undefined,
): ToolCallData[] {
  if (!parts) return [];
  return parts.flatMap((part) => {
    const tool = part?.type === "tool" ? part.tool : null;
    if (!tool || tool.toolName !== "generate_image") return [];
    return [
      {
        id: tool.id ?? part.id ?? "generated-image",
...
export function SubagentCard({ tool, className }: SubagentCardProps) {
  ...
  return (
    <View className={cn("gap-y-1", className)}>
      ...
      {generatedImageTools.length === 1 ? (
        <GenerateImageWidget tool={generatedImageTools[0]} />
      ) : generatedImageTools.length > 1 ? (
        <GeneratedImageGallery
          tools={generatedImageTools.map((imageTool) => ({
            tool: imageTool,
            id: imageTool.id,
          }))}
        />
